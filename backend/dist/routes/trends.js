 ...
 */
exports.trendsRouter.get('/keyword/:keyword', async (req, res) => {
    try {
 ...
 */
exports.trendsRouter.get('/years/range', async (req, res) => {
    try {
        const years = await Paper_1.PaperModel.distinct('year');
 ...
 */
exports.trendsRouter.post('/populate', async (req, res) => {
    try {
        const papers = await Paper_1.PaperModel.find().populate('keywords');
        const keywordsByYear = new Map();
        ...
        for (const [keyword, yearMap] of keywordsByYear) {
            for (const [year, { count, papers }] of yearMap) {
                await KeywordTrend_1.KeywordTrendModel.updateOne({ keyword, year }, { keyword, year, count, papers: [...new Set(papers)] }, { upsert: true });
                upserted++;
            }
        }
