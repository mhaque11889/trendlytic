...
const clustering_1 = require("../services/clustering");
const graphAnalysis_1 = require("../services/graphAnalysis");
 ...
 */
exports.reportsRouter.post('/cluster', async (req, res) => {
    try {
        const numberOfClusters = req.query.numberOfClusters
            ? parseInt(req.query.numberOfClusters)
            : 10;
        // Clear existing clusters
        await clustering_1.clusteringService.clearClusters();
        // Run clustering
        await clustering_1.clusteringService.clusterKeywords(numberOfClusters);
        const clusters = await clustering_1.clusteringService.getClusters();
        res.json({
            success: true,
 ...
 */
exports.reportsRouter.get('/clusters', async (req, res) => {
    try {
        const clusters = await KeywordCluster_1.KeywordClusterModel.find({}).sort({ size: -1 });
        // Fetch papers for each cluster
        const clustersWithPapers = await Promise.all(clusters.map(async (cluster) => {
 ...
 */
exports.reportsRouter.get('/clusters/:clusterId', async (req, res) => {
    try {
        ...
        const graph = await graphAnalysis_1.graphAnalysisService.getClusterGraph(clusterId);
        const statistics = graphAnalysis_1.graphAnalysisService.calculateGraphStatistics(graph);
        const centralNodes = graphAnalysis_1.graphAnalysisService.findCentralNodes(graph, 5);
        const communities = graphAnalysis_1.graphAnalysisService.findCommunities(graph);
        res.json({
            success: true,
            cluster,
            graph,
 ...
 */
exports.reportsRouter.post('/connectivity-graphs', async (req, res) => {
    try {
        // Clear existing connections
        await graphAnalysis_1.graphAnalysisService.clearConnections();
        // Build graphs for all clusters
        const graphs = await graphAnalysis_1.graphAnalysisService.buildAllClusterGraphs();
        res.json({
            success: true,
 ...
 */
exports.reportsRouter.get('/connectivity-graphs/:clusterId', async (req, res) => {
    try {
        const { clusterId } = req.params;
        const graph = await graphAnalysis_1.graphAnalysisService.getClusterGraph(clusterId);
        const statistics = graphAnalysis_1.graphAnalysisService.calculateGraphStatistics(graph);
 ...
 */
exports.reportsRouter.get('/connections', async (req, res) => {
    try {
        const limit = req.query.limit ? parseInt(req.query.limit) : 100;
        const skip = req.query.skip ? parseInt(req.query.skip) : 0;
        const clusterId = req.query.cluster_id;
        const query = clusterId ? { cluster_id: clusterId } : {};
        const connections = await KeywordConnection_1.KeywordConnectionModel.find(query)
            .sort({ co_occurrence_count: -1 })
            .skip(skip)
            .limit(limit);
        const total = await KeywordConnection_1.KeywordConnectionModel.countDocuments(query);
        res.json({
            success: true,
 ...
 */
exports.reportsRouter.post('/knowledge-graph', async (req, res) => {
    try {
        // Clear existing knowledge graphs
        await knowledgeGraph_1.knowledgeGraphService.clearKnowledgeGraphs();
        // Build unified knowledge graph
        await knowledgeGraph_1.knowledgeGraphService.buildUnifiedKnowledgeGraph();
        const graph = await knowledgeGraph_1.knowledgeGraphService.getKnowledgeGraph();
        res.json({
            success: true,
 ...
 */
exports.reportsRouter.get('/knowledge-graph', async (req, res) => {
    try {
        const graph = await knowledgeGraph_1.knowledgeGraphService.getKnowledgeGraph();
        if (!graph) {
            return res.status(404).json({
 ...
 */
exports.reportsRouter.post('/generate-all', async (req, res) => {
    try {
        ...
        const keywordLimit = req.query.keywordLimit
            ? parseInt(req.query.keywordLimit)
            : undefined;
        await clustering_1.clusteringService.clearClusters();
        await clustering_1.clusteringService.clusterKeywords(numberOfClusters, keywordLimit);
        const clusters = await clustering_1.clusteringService.getClusters();
        console.log(`Generated ${clusters.length} clusters`);
        // Step 2: Build connectivity graphs
        console.log('Step 2: Building connectivity graphs...');
        await graphAnalysis_1.graphAnalysisService.clearConnections();
        await graphAnalysis_1.graphAnalysisService.buildAllClusterGraphs();
        ...
        await knowledgeGraph_1.knowledgeGraphService.clearKnowledgeGraphs();
        await knowledgeGraph_1.knowledgeGraphService.buildUnifiedKnowledgeGraph();
        const knowledgeGraph = await knowledgeGraph_1.knowledgeGraphService.getKnowledgeGraph();
 ...
 */
exports.reportsRouter.get('/status', async (req, res) => {
    try {
        const clustersCount = await KeywordCluster_1.KeywordClusterModel.countDocuments({});
        const connectionsCount = await KeywordConnection_1.KeywordConnectionModel.countDocuments({});
        const knowledgeGraphs = await KnowledgeGraph_1.KnowledgeGraphModel.countDocuments({});
        res.json({
            success: true,
            status: {
 ...
 */
exports.reportsRouter.delete('/clear-all', async (req, res) => {
    try {
        await clustering_1.clusteringService.clearClusters();
        await graphAnalysis_1.graphAnalysisService.clearConnections();
        await knowledgeGraph_1.knowledgeGraphService.clearKnowledgeGraphs();
