...
function parseBufferToRows(originalname, buffer) {
    ...
    if (name.endsWith('.csv')) {
        ...
        return xlsx_1.default.utils.sheet_to_json(ws, { defval: '' });
    }
...
exports.importRouter.post('/upload', upload.single('file'), async (req, res) => {
    if (!req.file)
...
exports.importRouter.post('/process', upload.single('file'), async (req, res) => {
    if (!req.file)
        return res.status(400).json({ message: 'file is required' });
    try {
        const raw = parseBufferToRows(req.file.originalname, req.file.buffer);
        ...
        const result = await Paper_1.PaperModel.bulkWrite(ops, { ordered: false });
        ...
        for (const r of normalized) {
            ...
            for (let i = 0; i < authors.length; i++) {
                ...
                if (!name && !extId)
                    continue; // skip invalid author entry
                const key = extId ? `id:${extId}` : `name:${name}`;
                authorCounts.set(key, (authorCounts.get(key) || 0) + 1);
                if (!authorMeta.has(key))
                    authorMeta.set(key, { name: name || undefined });
            }
        ...
        if (authorOps.length)
            await Author_1.AuthorModel.bulkWrite(authorOps, { ordered: false });
        if (keywordOps.length)
            await Keyword_1.KeywordModel.bulkWrite(keywordOps, { ordered: false });
