...
import { graphAnalysisService } from './graphAnalysis';

interface GraphNode {
  id: string;
  label: string;
  type: 'keyword' | 'cluster' | 'theme';
  cluster_id?: string;
...
interface GraphEdge {
  ...
  target: string;
  weight: number;
  type: 'co-occurrence' | 'hierarchy' | 'related';
  properties?: Record<string, any>;
}
...
export class KnowledgeGraphService {
   ...
   */
  async buildUnifiedKnowledgeGraph(): Promise<void> {
    try {
      ...
      for (const cluster of clusters) {
        ...
        if (!nodeSet.has(clusterId)) {
          ...
          nodes.push({
            id: clusterId,
            label: (cluster.name || cluster.theme || 'Unknown') as string,
            type: 'cluster',
            properties: {
              size: cluster.size,
      ...
      const knowledgeGraph = new KnowledgeGraphModel({
        ...
        for (let i = 0; i < neighborList.length; i++) {
          for (let j = i + 1; j < neighborList.length; j++) {
            const edge = edges.find(
              (e) =>
                (e.source === neighborList[i] && e.target === neighborList[j]) ||
   ...
   */
  private calculateCentrality(nodes: GraphNode[], edges: GraphEdge[]): CentralityScores[] {
    const degrees = new Map<string, number>();
    ...
    // Combine scores
    const scores: CentralityScores[] = nodes
      .map((node) => ({
        node_id: node.id,
   ...
   */
  private detectCommunities(nodes: GraphNode[], edges: GraphEdge[]): GraphNode[][] {
   ...
   */
  private findHubNodes(nodes: GraphNode[], edges: GraphEdge[], topN: number): GraphNode[] {
    const degrees = new Map<string, number>();
   ...
   */
  private bfsDistances(startId: string, nodes: GraphNode[], edges: GraphEdge[]): Map<string, number> {
  ...
  async getKnowledgeGraph(): Promise<any> {
    const graphs = await KnowledgeGraphModel.find({}).sort({ created_at: -1 });
   ...
   */
  async clearKnowledgeGraphs(): Promise<void> {
